...
if DEBUG_FAST:
    ...
    DURACION_30MIN = 60   # antes 1800 s (30 min)
...
# Cada bloque = una tabla de la hoja. La clave "valor" indica QUE promedio se
# escribe en esa tabla:
#     "RMS"  -> promedio RMS de la ventana de compliance
#     "Peak" -> valor de pico (medido o calculado como RMS x sqrt(2))
#     None   -> tabla que se deja en blanco (la rellena el operador a mano)
EXCEL_COL_MAP = {
    "CE": [
        {   "valor": "RMS",
...
def enter(texto="Presiona Continuar para seguir..."):
    input(f"\n  --> {texto}")
...
def actualizar_excel(ruta_excel, region, nombre_posicion, distancias_cm,
                     promedios_por_tipo, technology, wpt_client, battery,
                     fechas_fin, frecuencia_khz):
    """
    ...
    Cada bloque de EXCEL_COL_MAP lleva una clave "valor" ("RMS", "Peak" o
    None) que determina que promedio se escribe en esa tabla. Los tres
    parametros siguientes son dicts indexados por ese tipo:

        distancias_cm      {"RMS": 1.0, "Peak": 1.0}
        promedios_por_tipo {"RMS": {...}, "Peak": {...}}
        fechas_fin         {"RMS": datetime, "Peak": datetime}

    Un bloque con "valor": None nunca se escribe. Un bloque cuyo tipo no
    tenga promedio disponible se salta con aviso.
    """
    global _EXCEL_PASSWORD
    ...
    freq_defecto = round(frecuencia_khz / 1000, 6)

    promedios_por_tipo = promedios_por_tipo or {}
    distancias_cm      = distancias_cm or {}
    fechas_fin         = fechas_fin or {}

    # Seleccion de bloques: solo los que tienen tipo asignado y datos disponibles
    a_escribir = []
    for cols in bloques:
        tipo = cols.get("valor")          # "RMS", "Peak" o None (tabla en blanco)
        if tipo is None:
            continue
        prom = promedios_por_tipo.get(tipo)
        if not prom:
            msg(f"  Excel '{sheet_name}': sin datos {tipo}, bloque col "
                f"{cols['technology']} no escrito.", "AVISO")
            continue
        a_escribir.append((cols, tipo, prom))

    if not a_escribir:
        msg(f"No hay datos que escribir en '{sheet_name}'.", "AVISO")
        wb.close()
        return

    # Misma fila para todos los bloques -> las tablas RMS y Peak quedan alineadas
    fila = max(encontrar_primera_fila_libre(ws, c["technology"], c["first_data_row"])
               for c, _, _ in a_escribir)

    for cols, tipo, prom in a_escribir:
        h_prom   = prom.get("h_prom")
        e_prom   = prom.get("e_prom")
        freq_mhz = prom.get("freq_mhz") or freq_defecto

        fecha     = fechas_fin.get(tipo)
        fecha_str = fecha.strftime("%Y-%m-%d") if hasattr(fecha, "strftime") else ""

        dist = distancias_cm.get(tipo)

        if tipo == "Peak" and prom.get("multi_frecuencia"):
            msg(f"  Excel '{sheet_name}': Peak multi-frecuencia, se escribe la fila "
                f"sin valores de campo (rellenar a mano).", "AVISO")

        escribir_fila_excel(
            ws, fila, cols, technology, wpt_client, battery,
            nombre_posicion, dist, freq_mhz, e_prom, fecha_str,
            freq_mhz, h_prom, fecha_str
        )
        msg(f"  Excel '{sheet_name}' bloque {tipo} (col {cols['technology']}): "
            f"fila {fila} escrita", "OK")
...
def realizar_fase(fase, nombre_posicion, carpeta_posicion, peak_rms_anterior, api_location):
    ...
    if peak_rms_actual != peak_rms_anterior:
        ...
        enter("Presiona Continuar cuando hayas cambiado el modo en MAGpy3")
...
def procesar_posicion(nombre_posicion, carpeta_raiz, regiones,
    ...
    enter("Presiona Continuar para INICIAR las medidas de esta posicion")
    ...
    fecha_fin_peak    = fecha_fin_rms   # se sobreescribe si hay fase Peak real
    if plan["necesita_peak"]:
        ...
        if distancia_peak_cm == distancia_cm:
            ...
            enter("Presiona Continuar cuando hayas cambiado a Peak y reposicionado la sonda")
            ...
            if not exito_peak:
                ...
                enter("Presiona Continuar cuando hayas vuelto a RMS")
    ...
    for r in regiones:
        ...
        if not (prom_rms or prom_peak):
            continue
        # Cada bloque de EXCEL_COL_MAP coge el promedio que le corresponde
        # segun su clave "valor" (ISED: tabla 1 = RMS, tabla 2 = Peak,
        # tabla 3 = en blanco).
        actualizar_excel(
            ruta_excel, r, nombre_posicion,
            {"RMS": distancia_cm,  "Peak": distancia_peak_cm},
            {"RMS": prom_rms,      "Peak": prom_peak},
            technology, wpt_client, battery,
            {"RMS": fecha_fin_rms, "Peak": fecha_fin_peak},
            frecuencia_khz
        )
...
def mostrar_config_manual(cfg):
    ...
    enter("Presiona Continuar cuando hayas configurado MAGpy3")
...
def main():
    ...
    if DEBUG_FAST:
        ...
        enter("Presiona Continuar para seguir en modo depuracion")
